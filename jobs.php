...
$query = "Select * from table_jobs ORDER BY id LIMIT 1 ";
...
function crawleData($url,$imageUrl,$rowID,$subCat){
    ...
    if(!empty($output)){
        ...
        $price = $deepXpath->query('//strong[@class="x-large margintop7 block not-arranged"]')->item(0)->nodeValue;
        ...
        $title = $titleRaw3." ".get_suffix($dec);

        print $title;
        if(!isset($price)){
            $price = "-";
        }
        $content = "Salary : ".$price." \nPost by : ".$name."Contact Number# : ".$phoneN."\nDec : ".$dec."\n".$location;
        $post = array();
